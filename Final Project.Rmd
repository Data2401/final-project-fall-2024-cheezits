...
This graph shows that most cheeses can be suitable for a keto diet since they are all below 13.40g.

Except for one, gjetost. 

```{r echo = FALSE}
cheese <- cheese %>% 
  mutate(total_fat = sat_fat + polysat_fat + monosat_fat)
  
keto <- cheese %>% ggplot(aes(total_fat, carb, text = type)) + geom_point(color = "lightpink") + labs(title = "Best Cheeses for a Ketogenic Diet")
ggplotly(keto)
```
...
From this graph, we are able to determine that mozzarella is the best when it comes to fiber. But why? 

When most cheeses have 0 fiber. The few with low fiber are cheeses with some fruit or vegetable in them.
...
```{r echo = FALSE}
high_risk <- cheese %>% ggplot(aes(x = sat_fat, y = kcal, col =  type)) + geom_point() + labs(title = "Worst Cheeses to Consume")
...
```{r echo = FALSE}
heart_health <- cheese %>% filter(sat_fat < 1 & polysat_fat <= 1.4) %>% ggplot(aes(x = sat_fat, y = polysat_fat, col =  type)) + geom_point() +labs(title = "Heart Healthy Cheeses to Consider")
...
```{r, echo = FALSE}
psf <- cheese %>% ggplot(aes(x = protein, y = sat_fat, col =  type)) + geom_point() + labs(title = "Protein V Saturated Fat")
...
# How does the balance between protein, fat, and carbohydrate content differ between types of cheese? For example, are high-protein cheeses generally lower in carbs?
```{r echo= FALSE}
puc <- cheese %>% ggplot(aes(x = protein, y = carb, col =  type)) + geom_point() + labs(title = "Protein V Carbs")
